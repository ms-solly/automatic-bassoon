  ...
  const Details: React.FC<DetailsProps> = ({ bio, location, website, isDarkMode }) => {
    return (
      <div className={`mt-6 p-6 rounded-xl shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} transition-all duration-300`}>
        ...
        <p className={`mt-2 text-lg ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}><strong>Location:</strong> {location}</p>
        <a href={website} target="_blank" rel="noopener noreferrer" className={`text-neon-blue hover:text-neon-pink text-lg`}>
          {website}
        </a>
      </div>
    );
